...
public class ChallengeSystem : MonoBehaviour
{
    ...
    public int GetCurrentValue(ChallengeDefinition challenge)
    {
        ...
        {
            ChallengeType.SupplierSpend => challenge.scope == ChallengeScope.Daily ? dailySupplierSpend : monthlyTrackedSupplierSpend,
            ChallengeType.FlowerPurchaseQuantity => challenge.scope == ChallengeScope.Daily ? dailyFlowerPurchaseQuantity : monthlyFlowerPurchaseQuantity,
            ChallengeType.DistinctFlowerPurchases => challenge.scope == ChallengeScope.Daily ? dailyFlowerProductKeys.Count : monthlyFlowerProductKeys.Count,
            ...
            ChallengeType.HybridUnlockedCount => hybridDevelopmentSystem != null ? hybridDevelopmentSystem.UnlockedHybridNames.Count : 0,
            _ => 0
        };
    }
    ...
    public bool TryClaim(ChallengeDefinition challenge)
    {
        ...
        int bonusShopRating = GetChallengeShopRatingBonus(challenge);
        if (bonusShopRating > 0)
            shopManager?.AddShopRating(bonusShopRating);
    ...
    public string GetRewardText(ChallengeDefinition challenge)
    {
        ...
        int bonusShopRating = GetChallengeShopRatingBonus(challenge);
        string bonusText = bonusShopRating > 0 ? $" ＋ 店評価 +{bonusShopRating:N0}" : string.Empty;

        return challenge.rewardType switch
        {
            ChallengeRewardType.Money => $"報酬：{challenge.rewardAmount:N0}円{bonusText}",
            ChallengeRewardType.ShopRating => $"報酬：店評価 +{challenge.rewardAmount + bonusShopRating:N0}",
            ChallengeRewardType.Wrapping => $"報酬：ラッピング ×{challenge.rewardAmount:N0}{bonusText}",
            _ => string.Empty
        };
    }

    private static int GetChallengeShopRatingBonus(ChallengeDefinition challenge)
    {
        if (challenge == null)
            return 0;
        ...
        return challenge.scope switch
        {
            ChallengeScope.Daily => 50,
            ChallengeScope.Monthly => 150,
            ChallengeScope.Lifetime => 100 * Mathf.Max(1, challenge.chainStage),
            _ => 0
        };
    }
    ...
    private void HandleSupplierProductPurchased(string productKey)
    {
        ...
        SyncDateIfNeeded();
        if (!dailyFlowerProductKeys.Contains(productKey)) dailyFlowerProductKeys.Add(productKey);
        if (!monthlyFlowerProductKeys.Contains(productKey)) monthlyFlowerProductKeys.Add(productKey);
        OnChanged?.Invoke();
    }
    ...
    {
        return shopManager != null ? shopManager.GameYear * 1000 + shopManager.DayOfYear * 17 + 7 : 7;
    }

    private int GetMonthlySeed()
    {
        return shopManager != null ? shopManager.GameYear * 100 + shopManager.CurrentMonth * 31 + 11 : 11;
    }
    ...
    private static ChallengeDefinition C(string id, ChallengeScope scope, string chainId, int stage, string title, string description, ChallengeType type, int target, ChallengeRewardType rewardType, int rewardAmount)
    {
    ...
    private void ResolveReferences()
    {
        ...
        if (furnitureSystem == null) furnitureSystem = FindFirstObjectByType<FurnitureSystem>();
        if (bouquetSystem == null) bouquetSystem = FindFirstObjectByType<BouquetSystem>();
        if (customerUI == null) customerUI = FindFirstObjectByType<CustomerUI>();
